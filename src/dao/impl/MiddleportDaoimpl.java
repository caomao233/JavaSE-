...
import dao.BaseDao;
import dao.MiddleportDao;
import entity.Subject;
import entity.Teacher;


public class MiddleportDaoimpl extends BaseDao implements MiddleportDao{
	...
	public void insertmiddleport1(Subject subject) {
		String sql="insert into middleport(id,name,checksir) value(?,?,'张之维')";
		try {
	...
	public void insertmiddleport2(Subject subject) {
		String sql="insert into middleport(id,name,checksir) value(?,?,'柳坤生')";
		try {
			conn = getConn(); // 得到数据库连接
			pstmt = conn.prepareStatement(sql); // 得到PreparedStatement对象
			pstmt.setInt(1, subject.getId());
	...
	public void insertmiddleport3(Subject subject) {
		String sql="insert into middleport(id,name,checksir) value(?,?,'周全')";
		try {
			conn = getConn(); // 得到数据库连接
			pstmt = conn.prepareStatement(sql); // 得到PreparedStatement对象
			pstmt.setInt(1, subject.getId());
	...
	public void deletemiddleport(Subject subject,Teacher teacher) {
		String sql="delete from middleport where middleport.id=? and checksir=?";
		try {
			conn = getConn(); // 得到数据库连接
			pstmt = conn.prepareStatement(sql); // 得到PreparedStatement对象
			pstmt.setInt(1, subject.getId());
	...
	//插入没必要传老师
	 //public String selectopenprot1()//这个东西是给老彭用的
	    ...
	    public List<Subject> getAllmiddletempsubject(Teacher teahcer) {
			...
			try {
			String preparedSql = "select * from middleport where checksir=?";
			conn = getConn(); // 得到数据库连接
